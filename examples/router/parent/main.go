...
func main() {
	...
	_ = ipc.HandleTyped(router, "child-hello", func(ctx context.Context, req ChildHello) (any, error) {
		select {
		case <-childCalled:
		default:
			close(childCalled)
		}
		return ParentHello{From: "parent", Note: "hi " + req.From + ", got your call"}, nil
	})

	ep, stdin, _, err := ipc.NewCmdEndpoint(cmd, router.Handler())
	if err != nil {
	...
	var pong Pong
	req, err := ipc.NewRequest("ping", Ping{Text: "hello from parent"})
	if err != nil {
		fmt.Fprintln(os.Stderr, "parent make request:", err)
		_ = stdin.Close()
		_ = cmd.Wait()
		os.Exit(1)
	}
	if err := ep.Call(ctx, req, &pong); err != nil {
...
func buildChild() (string, error) {
	...
	childPath := filepath.Join(binDir, "ipc-router-child")
	cmd := exec.Command("go", "build", "-o", childPath, "./examples/router/child")
